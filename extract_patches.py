...
def extract_patches_2d(img,patch_shape,step=[1.0,1.0],batch_first=False):
    print(img.size())
    print(patch_shape)
    patch_H, patch_W = patch_shape[3], patch_shape[2]
    print(patch_H)
    print(patch_W)
    if(img.size(1)<patch_H):
        num_padded_H_Top = (patch_H - img.size(2))//2
        num_padded_H_Bottom = patch_H - img.size(2) - num_padded_H_Top
        padding_H = nn.ConstantPad2d((0,0,num_padded_H_Top,num_padded_H_Bottom),0)
        img = padding_H(img)
    if(img.size(2)<patch_W):
    ...
    print(patches_fold_H.size())
    
    if((img.size(1) - patch_H) % step_int[0] != 0):
        patches_fold_H = torch.cat((patches_fold_H,img[:,:,-patch_H:,].unsqueeze(2)),dim=2)
    print(patches_fold_H.size())
